...
namespace _0826
{
    internal class PreprocessForCardDetection
    {
        ...
        public class CardCandidate
        {
            ...
            {
                return $"Card (신뢰도: {Confidence:F2}, 면적: {Area:F0}, " +
                       $"비율: {AspectRatio:F2})";
            }
        }
        ...
        /// 명함 검출을 위한 전처리 수행
        /// </summary>
        /// <param name="src">원본 이미지</param>
        /// <returns>전처리된 이미지</returns>
        public static Mat PreprocessForCardDetectionDemo(Mat src)
        {
            Console.WriteLine("1. 명함 검출용 전처리 시작...");
            ...
            // 3단계: 엣지 검출 (Canny) - 명함의 경계선 찾기
            double threshold1 = 10;
            double threshold2 = 50;

            //// 이미지 밝기에 따른 임계값 자동 조정
            //Scalar meanBrightness = Cv2.Mean(blurred);
            //if (meanBrightness.Val0 > 150) // 밝은 이미지
            //{
            //    threshold1 = 75;
            //    threshold2 = 200;
            //}
            //else if (meanBrightness.Val0 < 100) // 어두운 이미지
            //{
            //    threshold1 = 30;
            //    threshold2 = 100;
            //}
        ...
        public static List<CardCandidate> FindCardCandidates(Mat edges, Mat original)
        {
            ...
            double minArea = imageArea * 0.01; // 최소 1%
            double maxArea = imageArea * 0.8;  // 최대 80%

            for (int i = 0; i < contours.Length; i++)
            {
                var candidate = AnalyzeContourForCard(contours[i], original.Size());

                if (candidate != null && candidate.Area >= minArea &&
                    candidate.Area <= maxArea)
                {
                    candidates.Add(candidate);
                }
            }

            Console.WriteLine($"검출된 후보 수: {candidates.Count}");
            return candidates;
        }
        ...
        /// 최적의 명함 후보 선택
        /// </summary>
        /// <param name="candidates">명함 후보 리스트</param>
        /// <returns>최적 명함 후보</returns>
        ...
        /// </summary>
        /// <param name="original">원본 이미지</param>
        /// <param name="bestCandidate">최적 명함 후보</param>
        /// <returns>추출된 명함 이미지</returns>
        public static Mat ExtractCard(Mat original, CardCandidate bestCandidate)
        {
            Console.WriteLine("3. 명함 영역 추출 중...");
            ...
            Cv2.WarpPerspective(original, extractedCard, perspectiveMatrix,
        ...
        /// </summary>
        /// <param name="original">원본 이미지</param>
        /// <param name="candidates">모든 후보들</param>
        /// <param name="bestCandidate">최적 후보</param>
        public static void DrawDetectionResults(Mat original,
            List<CardCandidate> candidates, CardCandidate bestCandidate)
        {
            ...
            for (int i = 0; i < Math.Min(candidates.Count, 5); i++)
            {
                ...
                // 신뢰도 텍스트 표시
                string confidenceText = $"{candidate.Confidence:F2}";
                Cv2.PutText(result, confidenceText,
                    new Point((int)corners[0].X, (int)corners[0].Y - 10),
                    HersheyFonts.HersheySimplex, 0.6, Scalar.White, 2);
            }

            Cv2.ImShow("Card Detection Result", result);
        }

        /// <summary>
        /// 윤곽선을 분석하여 명함 후보로 평가
        /// </summary>
        /// <param name="contour">윤곽선</param>
        /// <param name="imageSize">이미지 크기</param>
        /// <returns>명함 후보 객체</returns>
        private static CardCandidate AnalyzeContourForCard(Point[] contour, Size imageSize)
        {
            ...
            candidate.AspectRatio = Math.Max(width, height) / Math.Min(width, height);
        ...
        /// 명함 후보의 신뢰도 계산
        /// </summary>
        /// <param name="candidate">명함 후보</param>
        /// <param name="imageSize">이미지 크기</param>
        /// <returns>신뢰도 (0~1)</returns>
        private static double CalculateConfidence(CardCandidate candidate, Size imageSize)
        {
            ...
            double ratioDiff = Math.Abs(candidate.AspectRatio - idealRatio);
        ...
        /// <summary>
        /// 네 개의 꼭짓점을 시계방향으로 정렬 (왼쪽 위부터 시작)
        /// </summary>
        /// <param name="corners">꼭짓점 배열</param>
        /// <returns>정렬된 꼭짓점 배열</returns>
